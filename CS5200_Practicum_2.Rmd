...
xslDoc = read_xml(xslPath,package="xslt")
author_raw = xml_xslt(xmlDoc,xslDoc)
authorship = unique(xmlToDataFrame(XML::xmlParse(article_raw)))
rownames(authorship) <- NULL
authorId = c()
articleId = c()
for(i in 1:nrow(authorship)){
  temp = authorship[i,]
  articleId = append(articleIds, article[which(temp$title==article$title),]$articleId)
  authorId = append(authorIds,author[which(temp$LastName==author$LastName
                       & temp$ForeName==author$ForeName),]$authorId)
}
authorship = cbind(articleId=rownames(authorship),articleId=articleId,authorship)
authorship = subset(authorship, select = 1:1)
